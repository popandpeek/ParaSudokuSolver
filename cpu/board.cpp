...
void Board::remove_potential_values_from_col(std::set<int> _vals, int col_start) {
	...
	remove_potential_values(_vals, col_start + 9);
	remove_potential_values(_vals, col_start + 18);
}
	...
	for (int sub_grid_row = 0; sub_grid_row < SUB_BOARD_DIM; sub_grid_row++) {
		for (int sub_grid_col = 0; sub_grid_col < SUB_BOARD_DIM; sub_grid_col++) {
			...
			if (SUB_BOARD_DIM == 3) {
				...
				// Now we do columns
				for (int col = 0; col < SUB_BOARD_DIM; col++) {
					std::set<int>cell_1 = get_potential_set(col * SUB_BOARD_SIZE + grid_start);
					std::set<int>cell_2 = get_potential_set(col * SUB_BOARD_SIZE + grid_start + SUB_BOARD_SIZE);
					std::set<int>cell_3 = get_potential_set(col * SUB_BOARD_SIZE + grid_start + SUB_BOARD_SIZE * 2);
